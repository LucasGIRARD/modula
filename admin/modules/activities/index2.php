    ...
    foreach ($moduleAction as $key => $value) {
        switch ($value) {
            ...
            case 'dbVars':
                ...
                switch ($action) {
                    case 'listing':
                        $donneesSQL = select($connection, "SELECT ra.id, ra.enabled, ra.name, rac.name AS category FROM resume_activities AS ra LEFT JOIN resume_activities_title AS rac ON ra.RESUME_ACTIVITIES_TITLE_id = rac.id ORDER BY rac.name, ra.name");

                        foreach ($donneesSQL as $value) {
                            $activities[$value['id']]['category'] = $value['category'];
                            $activities[$value['id']]['enabled'] = $value['enabled'];
                            $activities[$value['id']]['name'] = $value['name'];
                        }
            ...
            case 'listView':
                ...
                $contentView = 'modules/activities/view.php';

                break;
            case 'addView':
                $actionDisplay = "Ajouter";
                $contentView = 'modules/activities/view_add.php';
                break;
            case 'nextView':
                $actionDisplay = "Ajouter";
                $contentView = 'modules/activities/view_add_' . $nextStep . '.php';
                break;
            case 'modifyView':
                $actionDisplay = "Modifier";
                $contentView = 'modules/activities/view_modify.php';
                break;
            case 'parametersView':
                break;
            case 'writeFile':
